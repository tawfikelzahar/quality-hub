...
export default function XbarRChartPage() {
  ...
  async function exportExcel() {
    ...
    overview.table({
      ...
      rows: [
        ...
        { chart: 'R (Range)', cl: result.cl_r, ucl: result.ucl_r, lcl: result.lcl_r },
      ],
    });
    ...
    dataSheet.table({
      headers: [
        ...
        { header: 'Range', key: 'range', align: 'right', numFmt: '0.0000' },
      ],
      rows: subgroups.map((row, i) => {
        const r: Record<string, string | number> = { sg: i + 1, xbar: result.xbarVals[i], range: result.rangeVals[i] ?? '' }
  ...
  function exportPDF() {
    ...
    const ctx = createPdfReport(messages.pdfReportTitle, 'xbar_r');
    ...
    const limitRows: KVRow[] = [
      ...
      ['R CL', niceNum(result.cl_r)],
      ['R UCL', niceNum(result.ucl_r)],
      ['R LCL', niceNum(result.lcl_r)],
    ];
    const capRows: KVRow[] = [
    ...
    dataTable(
      ...
      [
        ...
        { header: 'RANGE', width: ctx.pageWidth - ctx.margin * 2 - 210, align: 'right' },
      ],
